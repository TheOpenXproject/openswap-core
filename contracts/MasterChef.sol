...
contract MasterChef is Ownable {
    ...
    event RewardChange(uint256 sushiPerBlock, uint256 Blocktime);
    ...
    function updateBlockIssuanceWithBlocktime()
        ...
        sushiPerBlock = currentBlocktime.mul(sushiPerSecond).div(1000);
        lastBlocktime = currentBlocktime;
        emit RewardChange(sushiPerBlock, currentBlocktime);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        if(isUserRegisty[msg.sender] != true){
            userRegistry.push(msg.sender);
            isUserRegisty[msg.sender] = true;
        }
        PoolInfo storage pool = poolInfo[_pid];
    ...
    public onlyCollector {
        uint256 _amount = 0; //Withdraw only Openswap tokens and not LP tokens
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        require(user.amount >= _amount, "withdraw: not good");
        updatePool(_pid);
